...
wall1 = GameObject(position=Vector3(-5,0,0), size=Vector3(1,1,10)).add_component(BoxCollider(), Rigidbody(isKinematic=True), Wall())
wall2 = GameObject(position=Vector3(5,0,0), size=Vector3(1,1,10)).add_component(BoxCollider(), Rigidbody(isKinematic=True), Wall())
wall3 = GameObject(position=Vector3(0,0,-5), size=Vector3(10,1,1)).add_component(BoxCollider(), Rigidbody(isKinematic=True), Wall())
wall4 = GameObject(position=Vector3(0,0,5), size=Vector3(10,1,1)).add_component(BoxCollider(), Rigidbody(isKinematic=True), Wall())
...
for i in range(2):
    scene2 = copy.deepcopy(main)
    scene2.transform.local_position = Vector3(0,i*4,0)
    scene2.set_default()
    scene.append(scene2)

Core.run_max_speed(scene, Render=False, MaxTime=60*60)
